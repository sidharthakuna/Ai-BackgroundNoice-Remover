"""
audio_io.py — High-fidelity 48kHz audio I/O with Mid/Side stereo encoding/decoding.
"""
...
def save_audio(path: str, audio: np.ndarray, sr: int = SAMPLE_RATE, subtype: str = "PCM_24"):
    """Saves audio safely to disk at 48000Hz with true-peak ceiling protection."""
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)

    if audio.ndim == 2:
        out_data = audio.T if audio.shape[0] == 2 else audio
    else:
        out_data = audio
...
def encode_mid_side(stereo_audio: np.ndarray):
    """Converts (left, right) to Mid/Side spatial domains."""
    left = stereo_audio[0]
...
def decode_mid_side(mid: np.ndarray, side: np.ndarray):
    """Reconstructs stereo from Mid/Side."""
