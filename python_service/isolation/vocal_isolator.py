"""
vocal_isolator.py — Memory-safe chunked Demucs vocal isolation.
Processes audio in small 4-second overlapping frames with Hanning crossfades,
ensuring peak container memory remains within Render's 512MB free tier budget.
"""
...
def apply_demucs_chunked(audio: np.ndarray, demucs_model, chunk_seconds: float = 4.0) -> np.ndarray:
    """
    Executes Demucs vocal isolation on CPU in 4-second chunks with overlap-add reconstruction.
    Accepts 1D (mono) or 2D (stereo, shape=(2, N)) float32 numpy arrays.
    """
    ...
    for i in range(num_chunks):
        ...
        with torch.inference_mode():
            # returns (batch, sources, channels, time) where source 3 = vocals
