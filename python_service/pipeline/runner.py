"""
runner.py — Complete 48kHz studio audio DSP pipeline coordinator.
Orchestrates audio loading, Mid/Side encoding, Demucs, high-pass filtering,
Wiener spectral gating, DeepFilterNet3 neural inference, dual-band VAD,
broadcast dynamics, 5-band voice mastering EQ, and EBU R128 loudness normalization.
"""
...
from python_service.config import SAMPLE_RATE, MODE_CONFIGS, CHUNK_STREAMING_THRESHOLD_SECONDS
from python_service.audio_io import load_audio, save_audio, encode_mid_side, decode_mid_side
from python_service.dsp import (
    apply_highpass,
...
from python_service.isolation import apply_demucs_chunked
from python_service.pipeline.chunker import process_audio_chunked_or_direct
from python_service.engine.memory_guard import memory_guard
...
def execute_pipeline(
    ...
    """
    Executes the full studio DSP pipeline from input file to output master WAV.
    Streams progress updates and respects cooperative cancellation tokens.
    ...
    for ch_name, ch_audio in channels_to_process:
        ...
        # Stage B: Wiener-style spectral gating (with chunking protection for large files)
        ...
        ch_gated = process_audio_chunked_or_direct(
            ...
            threshold_seconds=CHUNK_STREAMING_THRESHOLD_SECONDS
        )
        del ch_hp
        ...
        # Helper function for neural enhancement block
        ...
        # Match length to original channel length
        n = len(ch_audio)
